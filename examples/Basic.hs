...
craftNormalUser name fullname sshPubKey userShell' = do
  ...
  when (userShell' == bash) $ do
    ...
    unlessM (File.exists bashrcFP) $
      exec_ "cp" ["/etc/bash.bashrc", fromAbsFile bashrcFP]
    craft_ $
      file bashrcFP
      & ownerAndGroup .~ user
  return user
...
craftAdmin name fullname sshPubKey userShell' = do
  ...
  craft_ $
    file ($(mkAbsDir "/etc/sudoers.d")</>sudoersfn)
