...
runCraftLocal ce' = interpretCraft ce' run
 where
  ...
  run (FileRead _ fp next) =
    Trans.lift (BS.readFile $ fromAbsFile fp) >>= next
  run (FileWrite _ fp content next) =
    Trans.lift (BS.writeFile (fromAbsFile fp) content) >> next
  -- run (SourceFile ce src dest next) =
  --   run (Exec_ ce "/bin/cp" [(fromAbsFile src), (fromAbsFile dest)] next)
